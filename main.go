...
var viewFlag = []cli.Flag{
	...
	&cli.BoolFlag{
		...
		Action: func(context *cli.Context, b bool) error {
			if b {
				contentFunc = append(contentFunc, contentFilter.SizeEnabler().EnableSize(sizeUint, r))
				if _, ok := p.(*printer.Byline); !ok {
					p = printer.NewByline()
				}
			}
	...
	&cli.BoolFlag{
		...
		Action: func(context *cli.Context, b bool) error {
			if b {
				contentFilter.SizeEnabler().SetEnableTotal()
			}
			return nil
		},
	},
...
var displayFlag = []cli.Flag{
	...
	&cli.StringFlag{
		...
		Action: func(context *cli.Context, s string) error {
			...
			case "long", "l", "verbose":
				contentFunc = append(contentFunc, filter.EnableFileMode(r), contentFilter.SizeEnabler().EnableSize(sizeUint, r), contentFilter.EnableOwner(r), contentFilter.EnableGroup(r), filter.EnableTime(timeFormat, r, timeType))
				if _, ok := p.(*printer.Byline); !ok {
					p = printer.NewByline()
				}
	...
	&cli.BoolFlag{
		...
		Action: func(context *cli.Context, b bool) error {
			if b {
				...
				typeFunc = newFF
				contentFunc = append(contentFunc, filter.EnableFileMode(r), contentFilter.SizeEnabler().EnableSize(sizeUint, r), contentFilter.EnableGroup(r), filter.EnableTime(timeFormat, r, timeType))
				if _, ok := p.(*printer.Byline); !ok {
					p = printer.NewByline()
				}
			}
	...
	&cli.BoolFlag{
		...
		Action: func(context *cli.Context, b bool) error {
			if b {
				...
				typeFunc = newFF
				contentFunc = append(contentFunc, filter.EnableFileMode(r), contentFilter.SizeEnabler().EnableSize(sizeUint, r), contentFilter.EnableOwner(r), filter.EnableTime(timeFormat, r, timeType))
				if _, ok := p.(*printer.Byline); !ok {
					p = printer.NewByline()
				}
			}
	...
	&cli.BoolFlag{
		...
		Action: func(context *cli.Context, b bool) error {
			if b {
				...
				typeFunc = newFF
				contentFilter.SizeEnabler().SetEnableTotal()
				contentFunc = append(contentFunc, filter.EnableFileMode(r), contentFilter.SizeEnabler().EnableSize(sizeUint, r), contentFilter.EnableOwner(r))
				if !context.Bool("G") {
					contentFunc = append(contentFunc, contentFilter.EnableGroup(r))
				}
				contentFunc = append(contentFunc, filter.EnableTime(timeFormat, r, timeType))
...
There is NO WARRANTY, to the extent permitted by law.`,
		...
		Action: func(context *cli.Context) error {
			...
			if context.Bool("tree") {
				...
				final:
					...
					if total, ok := contentFilter.SizeEnabler().Total(); ok {
						p.Print(fmt.Sprintf("  total %s", contentFilter.SizeEnabler().Size2String(total, 0)))
					}
					p.Print(stringSlice...)
					...
					if i != len(path)-1 {
						...
						contentFilter.SizeEnabler().Reset()
					}
				}
			}
	...
	app.Flags = append(app.Flags, viewFlag...)
	app.Flags = append(app.Flags, displayFlag...)
	app.Flags = append(app.Flags, filteringFlag...)
	app.Flags = append(app.Flags, sortingFlags...)
