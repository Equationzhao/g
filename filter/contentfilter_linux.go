...
func EnableSize(size Size, renderer *render.Renderer) ContentOption {
	return func(info os.FileInfo) string {
		...
		case Auto:
			for i := B; i <= ZB; i++ {
				if v < 1000 {
					res = strconv.FormatFloat(v, 'f', 1, 64) + convert2Size(i)
...
func EnableOwner(renderer *render.Renderer) ContentOption {
	...
	return func(info os.FileInfo) string {
		...
		return renderer.Owner(fmt.Sprintf("%*s", longestOwner, name)) // fillBlank(name, longestOwner)
	}
}
...
func EnableGroup(renderer *render.Renderer) ContentOption {
	...
	return func(info os.FileInfo) string {
		...
		return renderer.Owner(fmt.Sprintf("%*s", longestGroup, name))
	}
}
